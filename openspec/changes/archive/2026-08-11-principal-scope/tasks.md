...
- [x] 6.4 Run `./scripts/check.sh` with the active change and record the exact passing test/spec counts in evidence.
- [x] 6.5 Archive `principal-scope`, verify there are no active changes, and run post-archive `./scripts/check.sh`.
- [x] 6.6 Confirm no version bump, release tag, session authentication, grant store, or Trust Tier runtime entered the diff.
