...
已發佈基線 `v1.0.0-beta.4`：承接 beta.3；GitHub Actions 改用 Node 24 runtime 的 action（`checkout@v5`、`setup-node@v5`、`setup-uv@v7`），OpenSpec job 使用 Node 22。無新安全語意。獨立真人 TTY 仍待後續 beta；GA 前補 hash chain 與剩餘 DoD。
...
*最後更新：2026-08-12 · 策略：Tool-First 漸進式 · 已發佈 v1.0.0-beta.4 · 開發方式：Spec-driven + OpenSpec*
